...
A quadratic function takes the form $q(x)=x^TAx+b^T+c$ for any $A\in\bR^{n\times n}, b\in\bR^n, c\in\bR$, where $x^TAx=\sum_{i,j} A_{ij} x_ix_j$
...
\begin{align*}
    x^TAx=\frac{x^TAx}{2}+\frac{x^TAx}{2}=\frac{x^TAx}{2}+\frac{x^TA^Tx}{2}=\frac{1}{@}x^T(A+A^T)x=x^TGx
\end{align*}
\end{proof}
...
(3) Otherwise, $q(x)$ has no global minimizer
